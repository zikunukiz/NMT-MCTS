...
'''
Is same as policy_net.py except that the policy and value networks don't share an encoder here.
'''
...
class PolicyValueNet():
    ...
    def __init__(self, main_params, path_to_policy=None, path_to_value=None,adamlr=1e-4):
        ...
        self.policy_encoder_output = None #keep this when get for this batch for first time
        self.value_encoder_output = None
        ...
        #now set value encoder to be same as policy encoder
        self.value_net.encoder = copy.deepcopy(self.policy_net.encoder)
        self.value_net.encoder_embedding = copy.deepcopy(self.policy_net.encoder_embedding)
    ...
    def forward(self, src_tensor, dec_input, processes, sentence_lens,req_grad=False):
        ...
        policy_encoder_output_sliced = None
        value_encoder_output_sliced = None
        if not self.policy_encoder_output is None:
            policy_encoder_output_sliced = self.policy_encoder_output[:,processes-1,:]
            value_encoder_output_sliced = self.value_encoder_output[:,processes-1,:]
        ...
        with torch.set_grad_enabled(req_grad):
            policy_output, policy_encoder_output_sliced = self.policy_net.forward(src_tensor, dec_input,
                                             src_key_padding_mask=src_key_padding_mask,
                                             tgt_mask=None, tgt_key_padding_mask=dec_key_padding_mask,
                                             memory_key_padding_mask=src_key_padding_mask,
                                             memory=policy_encoder_output_sliced)
            
            act_probs = F.softmax(policy_output[sentence_lens-1, batch_indices, :], dim=1)
            #the shape is (batch_size, vocab size)

            value_output, value_encoder_output_sliced = self.value_net.forward(src_tensor, dec_input,
                                          src_key_padding_mask=src_key_padding_mask,
                                          tgt_mask=None, tgt_key_padding_mask=dec_key_padding_mask,
                                          memory_key_padding_mask=src_key_padding_mask,
                                          memory=value_encoder_output_sliced)

            if self.policy_encoder_output is None:
                #make sure first run has all processes and in 
                #right order
                print('Set the encoder output')
                self.policy_encoder_output = policy_encoder_output_sliced.clone()
                self.value_encoder_output = value_encoder_output_sliced.clone()
    ...
    def train_step(self, src_input, dec_input, mcts_probs, actions,bleus):
        ...
        self.policy_encoder_output = None #don't want to use encoder output from previous iterations here
        self.value_encoder_output = None #don't want to use encoder output from previous iterations here
