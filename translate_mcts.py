...
from onmt.translate.mcts_sequence_fix_length_single import MCTSDecoder
...
class Translator(object):
    ...
    def translate(
        ...
        for i, batch in enumerate(data_iter):
            ...
            self.out_file.write(' '.join(batch_data)+'\n')
            self.out_file.flush()
            if i% 500 == 0:
                print('Batch: %d \t time cost: %.4f' % (i, time.time()-start_time))
            if i == 1000:
                exit()
            continue
    ...
    def _translate_random_sampling(
        ...
        # Inmport MCTS Decoder
        mcts_decoder = MCTSDecoder(self._tgt_bos_idx, mb_device, self._tgt_bos_idx,memory_bank, memory_lengths, self.model, c_puct=5, n_playout=25)
        predictions_results = []
        for step in range(max_length):
            # Shape: (1, B, 1)
            decoder_input = mcts_decoder.alive_seq[:, -1].view(1, -1, 1)
            word_id = mcts_decoder.get_action(decoder_input, batch, src_vocabs, src_map=src_map, step=step).view(-1, 1)
            predictions_results.append(word_id.cpu().item())
            mcts_decoder.alive_seq = torch.cat([mcts_decoder.alive_seq, word_id], -1)
            if predictions_results[-1] == self._tgt_eos_idx:
                break
            #if word.cpu().item() == self._tgt_eos_idx:
            #    break
        ...
        test_tgt_vocab = dict()
        for k, v in self._tgt_vocab.stoi.items():
            test_tgt_vocab[v] = k
        
        predictions_words = []
        for idx in predictions_results:
            idx = int(idx)
            predictions_words.append(test_tgt_vocab[idx])
            #print(test_tgt_vocab[idx]+' ',)
        #exit()
        #print(predictions_results)
        #results["scores"] = random_sampler.scores
        #results["predictions"] = [[torch.LongTensor(predictions_results).cuda().long()]]
    ...
    def _translate_batch(
        ...
        mcts_decoder = MCTSDecoder(memory_bank, memory_lengths, inp, model, c_puct=0, n_playout=3)
        #current_prediction = 
        for step in range(max_length):
            decoder_input = beam.current_predictions.view(1, -1, 1)
            #word = mcts_decoder.get_action(decoder_in, batch, src_vocabs, src_map=src_map, step=i)
            #attn = None
            '''log_probs, attn = self._decode_and_generate(
                decoder_input,
                memory_bank,
