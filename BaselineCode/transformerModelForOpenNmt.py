...
class TransformerModel(nn.Module):
    ...
    def __init__(self,d_model,nhead,num_encoder_layers,num_decoder_layers,
                dim_feedforward,dropout,activation,enc_embedding_mat,dec_embedding_mat,src_vocab_size,tgt_vocab_size):
        ...
        #print('d_model:{}, tgt_vocab_size: {}'.format(self.d_model,tgt_vocab_size))
        self.encoder_embedding = nn.Embedding(src_vocab_size,d_model)
        self.decoder_embedding = nn.Embedding(tgt_vocab_size,d_model)

        self._reset_parameters()

        #MAKE SURE EMBEDDING PARAMS NOT RESET (tested that aren't)
        #self.encoder_embedding = nn.Embedding.from_pretrained(torch.from_numpy(enc_embedding_mat),freeze=False)
        #self.decoder_embedding = nn.Embedding.from_pretrained(torch.from_numpy(dec_embedding_mat),freeze=False)
    ...
    def forward(self, src, tgt, src_key_padding_mask=None,tgt_mask=None, tgt_key_padding_mask=None,
                memory_key_padding_mask=None,memory=None):
        #first embed src and tgt (here tgt )
        if memory is None:
          #we're here is haven't already gotten encoder output at a previous step
          memory = self.encoder_embedding(src).double() * math.sqrt(self.d_model)
          #print('src shape: {}, memory shape: {}'.format(src.shape,memory.shape))
          memory = self.pos_encoder(memory).double()
          #print('shape to encoder: {}'.format(memory.shape))
        ...
        #if memory.size(1) != tgt2.size(1):
        #    raise RuntimeError("the batch number of src and tgt must be equal")

        #if memory.size(2) != self.d_model or tgt2.size(2) != self.d_model:
        ...
        #print('dec source size: {}, tgt_key_padding_mask size: {}'.format(tgt2.shape,tgt_key_padding_mask.shape))
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        #print('pe shape: {}, x shape: {}'.format(self.pe[:x.size(0),:].shape,x.shape))
        #print('pe shape: ',Variable(self.pe[:, :x.size(1)],requires_grad=False).shape)
        #print('actual pe shape: ',self.pe.shape)
        #print('our new shape: ',self.pe.transpose(0,1)[:x.size(0), :].shape)
...
class LabelSmoothing(nn.Module):
    ...
    def forward(self, x, target):
        ...
        """
        print('NEWESET SHape: ',mask.shape)
        print(mask.shape)
        print(mask[:3,:4])
        """
